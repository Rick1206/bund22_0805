...
?>
			<p><a href="enticement_photo.php?action=enticement&e_id=<?php echo $e_id;?>">Enticement照片</a> > 添加Enticement照片</p>
			<div class="blist">
				<div class="tit">添加Enticement照片</div>
				<form action="enticement_photo.php?action=addenticement" method="post" enctype="multipart/form-data" name="form1" id="form1">
				<div class="itm">
					<div class="tabs"></div>
					<table class="tb-02">
						<tbody>
	...
	<?php
			...
			$back_data = $db->query("INSERT INTO ".$ros->table('enticement_photo')." (enticement_id,name, photo, orderby) VALUES ('".param("id")."', '".$param['name'].
			"', '".$pic."', '".$param['orderby']."')");
			
			if($back_data) {
				show_msg('添加成功!','enticement_photo.php?action=addenticement&e_id='.$param['id']);  
			} else {
				show_msg('请重试!','enticement_photo.php?action=addenticement&e_id='.$param['id']);
			}
		}
	...
	?>
			<p>Enticement照片</p>
			<div class="blist">
				<div class="tit">Enticement照片列表<s><a href="enticement_photo.php?action=addenticement&e_id=<?php echo $e_id;?>">添加Enticement照片</a></s></div>
				<div class="itm">
				<form id="enticement_list" name="enticement_list" method="post" action="enticement_photo.php?action=enticement">
				<table class="tb-01">
					<thead>
						<tr>
							<th width="5%">删除</th>
					...
					<tbody>
						...
						$all_date_num = page_1::page_all_num('enticement_photo',$where);
						$page = isset($_GET['page']) ? $_GET['page'] : 1 ;
						$page_a = 8;
						$offset = ($page - 1) * $page_a;
						$query = $db->query("SELECT * FROM ".$ros->table('enticement_photo').$where." ORDER BY orderby, enticement_id LIMIT $offset, $page_a");
						...
						<tr>
							...
							<td><a href="enticement_photo.php?action=enticement&edit=<?php echo $this_sql['id'];?>&e_id=<?php echo $e_id;?>"><img src="./images/modify.gif" width="16" height="16" border="0" alt="<?php echo $_L['edit'];?>" title="<?php echo $_L['edit'];?>" /></a></td>
						</tr>
						<?php
						}
						?>
					...
					<tr>
					  ...
					  <?php  echo page::page_num($all_date_num,$page_a,$page,'enticement_photo.php?action=enticement&type_id='.$e_id);?>
					  </td>
					</tr>
				</table>
...
<?php
			...
			if (!isset($param['editSubmit'])) {
				$query = $db->query("SELECT * FROM ".$ros->table('enticement_photo')." WHERE id='".$_GET['edit']."'");
				if($this_sql = $db->fetch_array($query)) {

?>
				<p><a href="enticement_photo.php?action=enticement&e_id=<?php echo $e_id;?>">Enticement照片</a> > 修改Enticement照片</p>
				<div class="blist">
					<div class="tit">修改Enticement照片</div>
					<form id="form1" name="form1" method="post" action="enticement_photo.php?action=enticement&edit=<?php echo $_GET['edit'];?>" enctype="multipart/form-data">
					<div class="itm">
						<div class="tabs"></div>
						<table class="tb-02">
							<tbody>
		...
		<?php
				...
				$query = $db->query("SELECT photo FROM ".$ros->table('enticement_photo')." WHERE id='".$_GET['edit']."'");
				...
				if($attachment_s){
					...
					$db->query("UPDATE ".$ros->table('enticement_photo')." SET photo='".$pic."' WHERE id='".$_GET['edit']."'");
				}
				
				$back_data = $db->query("UPDATE ".$ros->table('enticement_photo')." SET name='".$param['name']."', orderby='".$param['orderby']."' WHERE id='".$_GET['edit']."'");
				
				if($back_data) {
					show_msg('修改成功!',('enticement_photo.php?action=enticement&e_id='.$param['id']));  
				} else {
					show_msg('请重试!',('enticement_photo.php?action=enticement&e_id='.$param['id']));
				}
			}
		} else {
			if(isset($param['del_enticement_a'])) {
				if( is_array($param['del_enticement_a'])) {
					foreach($param['del_enticement_a'] as $k) {
						$query = $db->query("SELECT photo FROM ".$ros->table('enticement_photo')." WHERE id='$k'");
						$this_sql = $db->fetch_array($query);
						@unlink($attachdir.$this_sql['photo']);
						//@unlink($attachdir.'s_'.$this_sql['photo']);
						$db->query("DELETE FROM ".$ros->table('enticement_photo')." WHERE id = '$k'");
					}
				}
			}
			...
			if(isset($param['cg_enticement_a'])) {
				if( is_array($param['cg_enticement_a'])) {
					foreach($param['cg_enticement_a'] as $key => $value) {
						$query_order = $db->query("SELECT enticement_id FROM ".$ros->table('enticement_photo')." WHERE id='".$key."' AND orderby='".$value."' LIMIT 0, 1");
						if (!$this_order = $db->fetch_array($query_order)) {
							$db->query("UPDATE ".$ros->table('enticement_photo')." SET orderby = '".$value."' WHERE id = '".$key."'");
						}
					}
				}
			}

			show_msg('更新成功!',('enticement_photo.php?action=enticement&e_id='.$param['id']));
		}
	}
?>
	</div>
	...
	?>
			...
			<div class="blist">
				...
				<table class="tb-01">
					...
					<tr>
					  <td height="25" class="page" id="page" colspan="4">
					  <?php  echo page::page_num($all_date_num,$page_a,$page,'enticement_photo.php?action=enticement&e_id='.$e_id);?>
					  </td>
					</tr>
				</table>
				<p style="padding-top:10px;">
