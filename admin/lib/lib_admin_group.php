...
function del_user_group($id)
{
	...
	else
	{
		...
		else
		{
			...
			$sql = 'DELETE FROM '.$ros->table('user_group').' WHERE g_id='.$id;
			$res = $db->query($sql);
			if($res)
			{
				return true;
...
function get_user_group($id)
{
	...
	{
		$sql = ' SELECT * FROM '.$ros->table('user_group') . ' WHERE g_id='.$id;
		$res = $db->getAll($sql);
		$nam = $res[0]['name'];
		return $nam;
	}
...
{
	global $db,$ros;
	$sql = ' SELECT * FROM '.$ros->table('user_group') . ' WHERE g_id='.$id. ' LIMIT 1';
...
function get_right_byId($id)
{
	...
	else
	{
		...
		else
		{
			...
			$sql = 'SELECT `g_id`,`right` FROM '.$ros->table('user_group'). ' WHERE g_id='.$group_id;
			$res = $db->getAll($sql);
			$r_l = $res[0]['right'];
			$r_l = explode(',',$r_l);
			if(count($r_l)!=0)
...
function  get_right_details($id)
{
	...
	if(!empty($id))
	{
		...
		$sql = 'SELECT `g_id`,`right`,`name`,`desc` FROM '.$ros->table('user_group'). ' WHERE g_id='.$id;
		$res = $db->getAll($sql);
		return $res;
	}
	else
