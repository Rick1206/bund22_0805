...
?>
			<p><a href="manage.php?action=manage">董事会与管理层</a> > 添加董事会与管理层</p>
			<div class="blist">
				<div class="tit">添加董事会与管理层</div>
				<form action="manage.php?action=addmanage" method="post" enctype="multipart/form-data" name="form1" id="form1">
				<div class="itm">
					<div class="tabs"></div>
					<table class="tb-02">
						<tbody>
							...
							<tr>
								...
								<td width="38%"><select name="type_id">
								  <?php
								  foreach ($_manage_type as $key => $value) {
								  ?>
								...
								<td width="12%">图片:</td>
								<td width="38%"><span class="input01"><input name="photo[]" type="file" /></span> ('gif','jpg','jpeg','png',尺寸:80*106px)</td>
							</tr>
							<tr>
								<td height="40">排序:</td>
	...
	<?php
			...
			$back_data = $db->query("INSERT INTO ".$ros->table('manage')." (name_en, name_cn, name_gb, type_id, photo, detail_en, detail_cn, detail_gb, description_en, description_cn, description_gb, orderby) VALUES ('".$param['name_en']."', '".$param['name_cn']."', '".$param['name_gb']."', '".$param['type_id']."', '".$pic."', '".$param['detail_en']."', '".$param['detail_cn']."', '".$param['detail_gb']."', '".$param['description_en']."', '".$param['description_cn']."', '".$param['description_gb']."', '".$param['orderby']."')");
			if($back_data) {
				show_msg('添加成功!','manage.php?action=addmanage');  
			} else {
				show_msg('请重试!','manage.php?action=addmanage');
			}
		}
	...
	?>
			<p>董事会与管理层</p>
			<div class="blist">
				<div class="tit">董事会与管理层列表<s><a href="manage.php?action=addmanage">添加董事会与管理层</a></s></div>
				<div class="itm">
				<form id="form1" name="form1" method="get" action="manage.php">
				...
				<form id="manage_list" name="manage_list" method="post" action="manage.php?action=manage">
				<table class="tb-01">
					<thead>
						<tr>
							<th width="5%">删除</th>
					...
					<tbody>
						...
						$all_date_num = page_1::page_all_num('manage',$where);
						//$last_page = intval (($all_date_num - 1) / $page_a) + 1; //获得总页数
						$page = isset($_GET['page']) ? $_GET['page'] : 1 ;
						$page_a = 8;
						$offset = ($page - 1) * $page_a;
						$query = $db->query("SELECT * FROM ".$ros->table('manage').$where." ORDER BY type_id, orderby, manage_id LIMIT $offset, $page_a");
						while($this_sql = $db->fetch_array($query)) {
						?>
						<tr>
							<td height="50"> <input type="checkbox" name="del_manage_a[]" id="del_manage_a" value="<?php echo $this_sql['manage_id'];?>" /></td>
							<td><?php echo $_manage_type[$this_sql['type_id']];?></td>
							...
							<td><span class="input01" style="width:20px;"><input name="cg_manage_a[<?php echo $this_sql['manage_id'];?>]" type="text" value="<?php echo $this_sql['orderby'];?>" style="width:20px;" /></span></td>
							<td><a href="manage.php?action=manage&edit=<?php echo $this_sql['manage_id'];?>"><img src="./images/modify.gif" width="16" height="16" border="0" alt="<?php echo $_L['edit'];?>" title="<?php echo $_L['edit'];?>" /></a></td>
						</tr>
						<?php
						}
						?>
					...
					<tr>
					  ...
					  <?php  echo page::page_num($all_date_num,$page_a,$page,'manage.php?action=manage&type_id='.$type_id);?>
					  </td>
					</tr>
				</table>
...
<?php
			...
			if (!isset($param['editSubmit'])) {
				$query = $db->query("SELECT * FROM ".$ros->table('manage')." WHERE manage_id='".$_GET['edit']."'");
				if($this_sql = $db->fetch_array($query)) {

?>
				<p><a href="manage.php?action=manage">董事会与管理层</a> > 修改董事会与管理层</p>
				<div class="blist">
					<div class="tit">修改董事会与管理层</div>
					<form id="form1" name="form1" method="post" action="manage.php?action=manage&edit=<?php echo $_GET['edit'];?>" enctype="multipart/form-data">
					<div class="itm">
						<div class="tabs"></div>
						<table class="tb-02">
							<tbody>
								...
								<tr>
									...
									<td width="12%">图片:</td>
									<td width="38%"><a href="<?php echo $attachdir.$this_sql['photo'];?>" target="_blank"><?php echo $this_sql['photo'];?></a><br /><span class="input01"><input name="photo[]" type="file" /></span> ('gif','jpg','jpeg','png',尺寸:80*106px)</td>
								</tr>
								<tr>
									<td height="40">排序:</td>
		...
		<?php
				...
				$query = $db->query("SELECT photo FROM ".$ros->table('manage')." WHERE manage_id='".$_GET['edit']."'");
				...
				if($attachment_s){
					...
					$db->query("UPDATE ".$ros->table('manage')." SET photo='".$pic."' WHERE manage_id='".$_GET['edit']."'");
				}
				
				$back_data = $db->query("UPDATE ".$ros->table('manage')." SET name_en='".$param['name_en']."', name_cn='".$param['name_cn']."', name_gb='".$param['name_gb']."', type_id='".$param['type_id']."', detail_en='".$param['detail_en']."', detail_cn='".$param['detail_cn']."', detail_gb='".$param['detail_gb']."', description_en='".$param['description_en']."', description_cn='".$param['description_cn']."', description_gb='".$param['description_gb']."', orderby='".$param['orderby']."' WHERE manage_id='".$_GET['edit']."'");
				if($back_data) {
					show_msg('修改成功!','manage.php?action=manage');  
				} else {
					show_msg('请重试!','manage.php?action=manage');
				}
			}
		} else {
			if(isset($param['del_manage_a'])) {
				if( is_array($param['del_manage_a'])) {
					foreach($param['del_manage_a'] as $k) {
						$query = $db->query("SELECT photo FROM ".$ros->table('manage')." WHERE manage_id='$k'");
						$this_sql = $db->fetch_array($query);
						@unlink($attachdir.$this_sql['photo']);
						$db->query("DELETE FROM ".$ros->table('manage')." WHERE manage_id = '$k'");
					}
				}
			}
			
			if(isset($param['cg_manage_a'])) {
				if( is_array($param['cg_manage_a'])) {
					foreach($param['cg_manage_a'] as $key => $value) {
						$query_order = $db->query("SELECT manage_id FROM ".$ros->table('manage')." WHERE manage_id='".$key."' AND orderby='".$value."' LIMIT 0, 1");
						if (!$this_order = $db->fetch_array($query_order)) {
							$db->query("UPDATE ".$ros->table('manage')." SET orderby = '".$value."' WHERE manage_id = '".$key."'");
						}
					}
				}
			}

			show_msg('更新成功!','manage.php?action=manage');
		}
	}
?>
	</div>
