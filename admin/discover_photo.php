...
?>
			<p><a href="discover_photo.php?action=discover&d_id=<?php echo $d_id;?>">Discover照片</a> > 添加Discover照片</p>
			<div class="blist">
				<div class="tit">添加Discover照片</div>
				<form action="discover_photo.php?action=adddiscover" method="post" enctype="multipart/form-data" name="form1" id="form1">
				<div class="itm">
					<div class="tabs"></div>
					<table class="tb-02">
						<tbody>
	...
	<?php
			...
			$back_data = $db->query("INSERT INTO ".$ros->table('discover_photo')." (discover_id,name, photo, orderby) VALUES ('".param("id")."', '".$param['name'].
			"', '".$pic."', '".$param['orderby']."')");
			
			if($back_data) {
				show_msg('添加成功!','discover_photo.php?action=adddiscover&d_id='.$param['id']);  
			} else {
				show_msg('请重试!','discover_photo.php?action=adddiscover&d_id='.$param['id']);
			}
		}
	...
	?>
			<p>Discover照片</p>
			<div class="blist">
				<div class="tit">Discover照片列表<s><a href="discover_photo.php?action=adddiscover&d_id=<?php echo $d_id;?>">添加Discover照片</a></s></div>
				<div class="itm">
				<form id="discover_list" name="discover_list" method="post" action="discover_photo.php?action=discover">
				<table class="tb-01">
					<thead>
						<tr>
							<th width="5%">删除</th>
					...
					<tbody>
						...
						$all_date_num = page_1::page_all_num('discover_photo',$where);
						$page = isset($_GET['page']) ? $_GET['page'] : 1 ;
						$page_a = 8;
						$offset = ($page - 1) * $page_a;
						$query = $db->query("SELECT * FROM ".$ros->table('discover_photo').$where." ORDER BY orderby, discover_id LIMIT $offset, $page_a");
						...
						<tr>
							...
							<td><a href="discover_photo.php?action=discover&edit=<?php echo $this_sql['id'];?>&d_id=<?php echo $d_id;?>"><img src="./images/modify.gif" width="16" height="16" border="0" alt="<?php echo $_L['edit'];?>" title="<?php echo $_L['edit'];?>" /></a></td>
						</tr>
						<?php
						}
						?>
...
<?php
			...
			if (!isset($param['editSubmit'])) {
				$query = $db->query("SELECT * FROM ".$ros->table('discover_photo')." WHERE id='".$_GET['edit']."'");
				if($this_sql = $db->fetch_array($query)) {

?>
				<p><a href="discover_photo.php?action=discover&d_id=<?php echo $d_id;?>">Discover照片</a> > 修改Discover照片</p>
				<div class="blist">
					<div class="tit">修改Discover照片</div>
					<form id="form1" name="form1" method="post" action="discover_photo.php?action=discover&edit=<?php echo $_GET['edit'];?>" enctype="multipart/form-data">
					<div class="itm">
						<div class="tabs"></div>
						<table class="tb-02">
							<tbody>
		...
		<?php
				...
				$query = $db->query("SELECT photo FROM ".$ros->table('discover_photo')." WHERE id='".$_GET['edit']."'");
				...
				if($attachment_s){
					...
					$db->query("UPDATE ".$ros->table('discover_photo')." SET photo='".$pic."' WHERE id='".$_GET['edit']."'");
				}
				
				$back_data = $db->query("UPDATE ".$ros->table('discover_photo')." SET name='".$param['name']."', orderby='".$param['orderby']."' WHERE id='".$_GET['edit']."'");
				
				if($back_data) {
					show_msg('修改成功!',('discover_photo.php?action=discover&d_id='.$param['id']));  
				} else {
					show_msg('请重试!',('discover_photo.php?action=discover&d_id='.$param['id']));
				}
			}
		} else {
			if(isset($param['del_discover_a'])) {
				if( is_array($param['del_discover_a'])) {
					foreach($param['del_discover_a'] as $k) {
						$query = $db->query("SELECT photo FROM ".$ros->table('discover_photo')." WHERE id='$k'");
						$this_sql = $db->fetch_array($query);
						@unlink($attachdir.$this_sql['photo']);
						//@unlink($attachdir.'s_'.$this_sql['photo']);
						$db->query("DELETE FROM ".$ros->table('discover_photo')." WHERE id = '$k'");
					}
				}
			}
			...
			if(isset($param['cg_discover_a'])) {
				if( is_array($param['cg_discover_a'])) {
					foreach($param['cg_discover_a'] as $key => $value) {
						$query_order = $db->query("SELECT discover_id FROM ".$ros->table('discover_photo')." WHERE id='".$key."' AND orderby='".$value."' LIMIT 0, 1");
						if (!$this_order = $db->fetch_array($query_order)) {
							$db->query("UPDATE ".$ros->table('discover_photo')." SET orderby = '".$value."' WHERE id = '".$key."'");
						}
					}
				}
			}

			show_msg('更新成功!',('discover_photo.php?action=discover&d_id='.$param['id']));
		}
	}
?>
	</div>
