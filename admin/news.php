...
?>
			<p><a href="news.php?action=news">公司资讯</a> > 添加公司资讯</p>
			<div class="blist">
				<div class="tit">添加公司资讯</div>
				<form action="news.php?action=addnews" method="post" enctype="multipart/form-data" name="form1" id="form1">
				<div class="itm">
					<div class="tabs"></div>
					<table class="tb-02">
						<tbody>
	...
	<?php
			...
			$back_data = $db->query("INSERT INTO ".$ros->table('news')." (name_en, name_cn, name_gb, pdf_en, pdf_cn, pdf_gb, dateline) VALUES ('".$param['name_en']."', '".$param['name_cn']."', '".$param['name_gb']."', '".$file_en."', '".$file_cn."', '".$file_gb."', '".$param['dateline']."')");
			if($back_data) {
				show_msg('添加成功!','news.php?action=addnews');  
			} else {
				show_msg('请重试!','news.php?action=addnews');
			}
		}
	} elseif($action == "news") {
	...
	?>
			<p>公司资讯</p>
			<div class="blist">
				<div class="tit">公司资讯列表<s><a href="news.php?action=addnews">添加公司资讯</a></s></div>
				<div class="itm">
				<form id="news_list" name="news_list" method="post" action="news.php?action=news">
				<table class="tb-01">
					<thead>
						<tr>
							<th width="5%">删除</th>
					...
					<tbody>
						...
						$all_date_num = page_1::page_all_num('news','');
						//$last_page = intval (($all_date_num - 1) / $page_a) + 1; //获得总页数
						$page = isset($_GET['page']) ? $_GET['page'] : 1 ;
						$page_a = 8;
						$offset = ($page - 1) * $page_a;
						$query = $db->query("SELECT * FROM ".$ros->table('news')." ORDER BY dateline DESC, news_id DESC LIMIT $offset, $page_a");
						while($this_sql = $db->fetch_array($query)) {
						?>
						<tr>
							<td height="50"> <input type="checkbox" name="del_news_a[]" id="del_news_a" value="<?php echo $this_sql['news_id'];?>" /></td>
							<td><?php echo $this_sql['name_cn'] ? $this_sql['name_cn'] : $this_sql['name_en'];?></td>
							<td><?php echo $this_sql['dateline'];?></td>
							<td><a href="news.php?action=news&edit=<?php echo $this_sql['news_id'];?>"><img src="./images/modify.gif" width="16" height="16" border="0" alt="<?php echo $_L['edit'];?>" title="<?php echo $_L['edit'];?>" /></a></td>
						</tr>
						<?php
						}
						?>
					...
					<tr>
					  ...
					  <?php  echo page::page_num($all_date_num,$page_a,$page,'news.php?action=news');?>
					  </td>
					</tr>
				</table>
...
<?php
			...
			if (!isset($param['editSubmit'])) {
				$query = $db->query("SELECT * FROM ".$ros->table('news')." WHERE news_id='".$_GET['edit']."'");
				if($this_sql = $db->fetch_array($query)) {

?>
				<p><a href="news.php?action=news">公司资讯</a> > 修改公司资讯</p>
				<div class="blist">
					<div class="tit">修改公司资讯</div>
					<form id="form1" name="form1" method="post" action="news.php?action=news&edit=<?php echo $_GET['edit'];?>" enctype="multipart/form-data">
					<div class="itm">
						<div class="tabs"></div>
						<table class="tb-02">
							<tbody>
		...
		<?php
				...
				$query = $db->query("SELECT pdf_en, pdf_cn, pdf_gb FROM ".$ros->table('news')." WHERE news_id='".$_GET['edit']."'");
				...
				if($attachment_s){
					...
					$db->query("UPDATE ".$ros->table('news')." SET pdf_en='".$file_en."' WHERE news_id='".$_GET['edit']."'");
				}
				$attachment_s = ($attachments_arg_s = attach_upload(array('pdf'), 'pdf_cn')) ? 1 : 0;
				if($attachment_s){
					$file_cn = $attachments_arg_s[0]['attachment'];
					@unlink($attachdir.$this_sql['pdf_cn']);
					$db->query("UPDATE ".$ros->table('news')." SET pdf_cn='".$file_cn."' WHERE news_id='".$_GET['edit']."'");
				}
				$attachment_s = ($attachments_arg_s = attach_upload(array('pdf'), 'pdf_gb')) ? 1 : 0;
				if($attachment_s){
					$file_gb = $attachments_arg_s[0]['attachment'];
					@unlink($attachdir.$this_sql['pdf_gb']);
					$db->query("UPDATE ".$ros->table('news')." SET pdf_gb='".$file_gb."' WHERE news_id='".$_GET['edit']."'");
				}
				
				$back_data = $db->query("UPDATE ".$ros->table('news')." SET name_en='".$param['name_en']."', name_cn='".$param['name_cn']."', name_gb='".$param['name_gb']."', dateline='".$param['dateline']."' WHERE news_id='".$_GET['edit']."'");
				if($back_data) {
					show_msg('修改成功!','news.php?action=news');  
				} else {
					show_msg('请重试!','news.php?action=news');
				}
			}
		} else {
			if(isset($param['del_news_a'])) {
				if( is_array($param['del_news_a'])) {
					foreach($param['del_news_a'] as $k) {
						$query = $db->query("SELECT pdf_en, pdf_cn, pdf_gb FROM ".$ros->table('news')." WHERE news_id='$k'");
						$this_sql = $db->fetch_array($query);
						@unlink($attachdir.$this_sql['pdf_en']);
						@unlink($attachdir.$this_sql['pdf_cn']);
						@unlink($attachdir.$this_sql['pdf_gb']);
						$db->query("DELETE FROM ".$ros->table('news')." WHERE news_id = '$k'");
					}
				}
			}

			show_msg('更新成功!','news.php?action=news');
		}
	}
?>
	</div>
