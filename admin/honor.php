...
?>
			<p><a href="honor.php?action=honor">企业荣誉</a> > 添加企业荣誉</p>
			<div class="blist">
				<div class="tit">添加企业荣誉</div>
				<form action="honor.php?action=addhonor" method="post" enctype="multipart/form-data" name="form1" id="form1">
				<div class="itm">
					<div class="tabs"></div>
					<table class="tb-02">
						<tbody>
							...
							<tr>
								...
								<td width="12%">图片:</td>
								<td width="38%"><span class="input01"><input name="photo[]" type="file" /></span> ('gif','jpg','jpeg','png',尺寸:228*300px)</td>
							</tr>
							<tr>
								<td colspan="4" height="10"></td>     
							</tr>
	...
	<?php
			...
			$back_data = $db->query("INSERT INTO ".$ros->table('honor')." (name_en, name_cn, name_gb, photo, dateline) VALUES ('".$param['name_en']."', '".$param['name_cn']."', '".$param['name_gb']."', '".$pic."', '".$param['dateline']."')");
			if($back_data) {
				show_msg('添加成功!','honor.php?action=addhonor');  
			} else {
				show_msg('请重试!','honor.php?action=addhonor');
			}
		}
	} elseif($action == "honor") {
	...
	?>
			<p>企业荣誉</p>
			<div class="blist">
				<div class="tit">企业荣誉列表<s><a href="honor.php?action=addhonor">添加企业荣誉</a></s></div>
				<div class="itm">
				<form id="honor_list" name="honor_list" method="post" action="honor.php?action=honor">
				<table class="tb-01">
					<thead>
						<tr>
							<th width="5%">删除</th>
					...
					<tbody>
						...
						$query = $db->query("SELECT * FROM ".$ros->table('honor')." ORDER BY dateline DESC, honor_id DESC LIMIT $offset, $page_a");
						while($this_sql = $db->fetch_array($query)) {
						?>
						<tr>
							<td height="50"> <input type="checkbox" name="del_honor_a[]" id="del_honor_a" value="<?php echo $this_sql['honor_id'];?>" /></td>
							<td><?php echo $this_sql['name_cn'] ? $this_sql['name_cn'] : $this_sql['name_en'];?></td>
							<td><?php echo $this_sql['dateline'];?></td>
							<td><a href="honor.php?action=honor&edit=<?php echo $this_sql['honor_id'];?>"><img src="./images/modify.gif" width="16" height="16" border="0" alt="<?php echo $_L['edit'];?>" title="<?php echo $_L['edit'];?>" /></a></td>
						</tr>
						<?php
						}
						?>
					...
					<tr>
					  ...
					  <?php  echo page::page_num($all_date_num,$page_a,$page,'honor.php?action=honor');?>
					  </td>
					</tr>
				</table>
...
<?php
			...
			if (!isset($param['editSubmit'])) {
				$query = $db->query("SELECT * FROM ".$ros->table('honor')." WHERE honor_id='".$_GET['edit']."'");
				if($this_sql = $db->fetch_array($query)) {

?>
				<p><a href="honor.php?action=honor">企业荣誉</a> > 修改企业荣誉</p>
				<div class="blist">
					<div class="tit">修改企业荣誉</div>
					<form id="form1" name="form1" method="post" action="honor.php?action=honor&edit=<?php echo $_GET['edit'];?>" enctype="multipart/form-data">
					<div class="itm">
						<div class="tabs"></div>
						<table class="tb-02">
							<tbody>
								...
								<tr>
									...
									<td width="12%">图片:</td>
									<td width="38%"><a href="<?php echo $attachdir.$this_sql['photo'];?>" target="_blank"><?php echo $this_sql['photo'];?></a><br /><span class="input01"><input name="photo[]" type="file" /></span> ('gif','jpg','jpeg','png',尺寸:228*300px)</td>
								</tr>
								<tr>
									<td colspan="4" height="10"></td>     
								</tr>
		...
		<?php
				...
				$query = $db->query("SELECT photo FROM ".$ros->table('honor')." WHERE honor_id='".$_GET['edit']."'");
				...
				if($attachment_s){
					...
					$db->query("UPDATE ".$ros->table('honor')." SET photo='".$pic."' WHERE honor_id='".$_GET['edit']."'");
				}
				
				$back_data = $db->query("UPDATE ".$ros->table('honor')." SET name_en='".$param['name_en']."', name_cn='".$param['name_cn']."', name_gb='".$param['name_gb']."', dateline='".$param['dateline']."' WHERE honor_id='".$_GET['edit']."'");
				if($back_data) {
					show_msg('修改成功!','honor.php?action=honor');  
				} else {
					show_msg('请重试!','honor.php?action=honor');
				}
			}
		} else {
			if(isset($param['del_honor_a'])) {
				if( is_array($param['del_honor_a'])) {
					foreach($param['del_honor_a'] as $k) {
						$query = $db->query("SELECT photo FROM ".$ros->table('honor')." WHERE honor_id='$k'");
						$this_sql = $db->fetch_array($query);
						@unlink($attachdir.$this_sql['photo']);
						$db->query("DELETE FROM ".$ros->table('honor')." WHERE honor_id = '$k'");
					}
				}
			}
			
			show_msg('更新成功!','honor.php?action=honor');
		}
	}
?>
	</div>
